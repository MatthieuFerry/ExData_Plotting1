...
# Sets the device to have 4 graphs
par(mfrow=c(2,2))

# first plot, same as in plot2.R, except ylab label
plot(hpc$Date, hpc$Global_active_power, type = "l", ylab = "Global Active Power", xlab = "", col="black")

# second plot : voltage over time
plot(hpc$Date, hpc$Voltage, type = "l", ylab = "Voltage", xlab = "datetime", col="black")

# third plot, same as in plot3.R, except legend border removed
...
legend("topright", lwd = 1, col = c("black","red","blue"), legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),bty = "n",title.adj = 0.5)

# 4th plot : Global_reactive_power over time
plot(hpc$Date, hpc$Global_reactive_power, type = "l", ylab = "Global_reactive_power", xlab = "datetime", col="black")
